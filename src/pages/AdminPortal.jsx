  ...
  <AnimatePresence>
    {isOpen && (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-2xl mx-4 relative max-h-[90vh] overflow-y-auto">
          ...
          <h2 className="text-2xl font-bold text-space-blue mb-6 border-b pb-4">{title}</h2>
          {children}
        </motion.div>
      </motion.div>
    )}
...
const HREmailComposer = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8">
      <div className="flex-grow bg-white p-8 rounded-2xl shadow-lg">
        <h3 className="text-2xl font-bold text-space-blue mb-6 border-b pb-4">Email Composer</h3>
        <form onSubmit={handleSend} className="space-y-4">
          ...
          <div><label className="block text-xs font-medium text-gray-500 mb-1">Message</label><textarea rows="8" required value={formData.message} onChange={e => setFormData({...formData, message: e.target.value})} className="w-full border-gray-300 rounded-lg px-3 py-2 border text-sm"></textarea></div>
          <button type="submit" className="bg-mint-green text-space-blue px-8 py-2 rounded-lg font-bold hover:bg-opacity-90"><i className="bi bi-send mr-2"></i> Send Email</button>
        </form>
      </div>

      <div className="w-full lg:w-1/3 space-y-6">
        <div className="bg-white p-6 rounded-2xl shadow-lg">
          <h4 className="font-bold text-space-blue mb-4">Email Templates</h4>
          <ul className="space-y-3">
            {templates.map(t => (
              <li key={t.id} onClick={() => handleTemplateClick(t)} className="p-3 bg-gray-50 hover:bg-mint-green hover:text-space-blue cursor-pointer rounded-lg text-sm transition-colors border border-gray-200 relative group">
                ...
                <div className="absolute top-2 right-2 hidden group-hover:flex gap-2">
                  <button onClick={(e) => { e.stopPropagation(); setEditingTemplate(t); setIsModalOpen(true); }} className="text-blue-500"><i className="bi bi-pencil-square"></i></button>
                  <button onClick={(e) => deleteTemplate(t.id, e)} className="text-red-500"><i className="bi bi-trash"></i></button>
                </div>
              </li>
            ))}
          </ul>
          <button onClick={() => { setEditingTemplate(null); setIsModalOpen(true); }} className="mt-4 w-full border-2 border-dashed border-gray-300 py-2 rounded-lg text-gray-500 text-sm hover:border-mint-green hover:text-mint-green transition-colors">+ Create Template</button>
        </div>
      </div>
...
const HRDashboardHome = () => (
  <>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      <div className="bg-white p-6 rounded-2xl shadow-md border-l-4 border-mint-green">
        <h3 className="text-gray-500 font-medium">Total Applicants</h3><p className="text-3xl font-bold text-space-blue mt-2">156</p>
      </div>
      <div className="bg-white p-6 rounded-2xl shadow-md border-l-4 border-blue-500">
        <h3 className="text-gray-500 font-medium">Active Job Posts</h3><p className="text-3xl font-bold text-space-blue mt-2">12</p>
      </div>
      <div className="bg-white p-6 rounded-2xl shadow-md border-l-4 border-yellow-500">
        <h3 className="text-gray-500 font-medium">Pending Interviews</h3><p className="text-3xl font-bold text-space-blue mt-2">8</p>
      </div>
    </div>
    
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <div className="bg-white p-6 rounded-2xl shadow-md">
        <h4 className="font-bold text-space-blue mb-4">Application Trends (Last 7 Days)</h4>
        <div className="h-48 border-b border-l border-gray-300 flex items-end gap-2 p-2 relative">
        ...
        {showGoal && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, height: 0 }} className="bg-yellow-50 border border-yellow-200 p-4 rounded-xl mb-6 flex justify-between items-center shadow-sm">
            <div className="flex items-center gap-3">
              <i className="bi bi-trophy text-yellow-500 text-2xl"></i>
              <div>
                <h4 className="font-bold text-yellow-800">Goal: Reach #1 Top Employer!</h4>
                <p className="text-sm text-yellow-700">Complete 3 more successful hires to earn the Top Employer Badge on public search.</p>
              </div>
            </div>
            <button onClick={() => setShowGoal(false)} className="text-yellow-600 hover:text-yellow-800"><i className="bi bi-x-lg"></i></button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-8 rounded-2xl shadow-lg">
          ...
          <div className="mb-6 flex items-center gap-6">
            <div className="w-24 h-24 bg-gray-100 border-2 border-dashed border-gray-300 rounded-xl flex items-center justify-center overflow-hidden relative">
            ...
            <div>
              ...
              <input type="file" onChange={handleLogoUpload} accept="image/jpeg, image/png" className="text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-mint-green file:text-space-blue hover:file:bg-opacity-80 cursor-pointer" />
              <p className="text-xs text-gray-400 mt-1">Accepts .JPEG, .PNG. Max size: 5MB.</p>
            </div>
          </div>
          ...
          <form onSubmit={e => { e.preventDefault(); addAlert('Profile updated.', 'success'); }} className="space-y-4">
            ...
            <button type="submit" className="bg-space-blue text-white px-6 py-2 rounded-lg font-bold">Save Profile</button>
          </form>
        </div>
...
const HREmployeeApplications = () => (
  <div className="bg-white p-8 rounded-2xl shadow-lg">
    <h3 className="text-2xl font-bold text-space-blue mb-6 border-b pb-4">Employee Applications</h3>
    <div className="overflow-x-auto">
      <table className="w-full text-left border-collapse">
        <thead><tr className="bg-gray-50 text-gray-600 text-sm uppercase border-b"><th className="p-4">Applicant</th><th className="p-4">Applied For</th><th className="p-4">Status</th><th className="p-4 text-center">Actions</th></tr></thead>
        <tbody className="divide-y divide-gray-100 text-sm">
          <tr><td className="p-4 font-bold text-space-blue">Alice Smith</td><td className="p-4">UI/UX Designer</td><td className="p-4"><span className="bg-yellow-100 text-yellow-800 px-2 py-1 rounded text-xs font-bold">Under Review</span></td><td className="p-4 text-center"><button className="bg-space-blue text-white px-3 py-1 rounded text-xs">View Profile</button></td></tr>
          <tr><td className="p-4 font-bold text-space-blue">Bob Jones</td><td className="p-4">Frontend Developer</td><td className="p-4"><span className="bg-green-100 text-green-800 px-2 py-1 rounded text-xs font-bold">Interviewed</span></td><td className="p-4 text-center"><button className="bg-space-blue text-white px-3 py-1 rounded text-xs">View Profile</button></td></tr>
        </tbody>
      </table>
    </div>
...
const HREmployeeManagement = () => (
  <div className="bg-white p-8 rounded-2xl shadow-lg">
    <h3 className="text-2xl font-bold text-space-blue mb-6 border-b pb-4">Employee Management (Hired)</h3>
    <div className="overflow-x-auto">
      <table className="w-full text-left border-collapse">
        <thead><tr className="bg-gray-50 text-gray-600 text-sm uppercase border-b"><th className="p-4">Employee</th><th className="p-4">Role</th><th className="p-4">Hired Date</th><th className="p-4 text-center">Actions</th></tr></thead>
        <tbody className="divide-y divide-gray-100 text-sm">
          <tr><td className="p-4 font-bold text-space-blue">Charlie Davis</td><td className="p-4">Warehouse Manager</td><td className="p-4">Sept 1, 2023</td><td className="p-4 text-center"><button className="bg-gray-200 px-3 py-1 rounded text-xs font-bold">201 File</button></td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
const HRQuestionsModule = () => {
  ...
  return (
    <div className="bg-white p-8 rounded-2xl shadow-lg">
      <div className="flex justify-between items-center mb-6 border-b pb-4">
        <h3 className="text-2xl font-bold text-space-blue">Questions Management</h3>
        <div className="flex gap-2">
          <button onClick={() => setActiveTab('banks')} className={`px-4 py-2 rounded-lg font-bold text-sm ${activeTab === 'banks' ? 'bg-space-blue text-white' : 'bg-gray-100 text-gray-600'}`}>Question Banks</button>
          <button onClick={() => setActiveTab('creator')} className={`px-4 py-2 rounded-lg font-bold text-sm ${activeTab === 'creator' ? 'bg-space-blue text-white' : 'bg-gray-100 text-gray-600'}`}>Question Creator</button>
        </div>
      </div>

      {activeTab === 'banks' && (
        <div>
          <div className="mb-4"><input type="text" placeholder="Search templates..." className="border rounded-lg px-4 py-2 text-sm w-1/3" /></div>
          <table className="w-full text-left border-collapse border">
            <thead><tr className="bg-gray-50 text-gray-600 text-sm border-b"><th className="p-4">Template Name</th><th className="p-4">Questions</th><th className="p-4 text-center">Actions</th></tr></thead>
            <tbody>
              <tr className="border-b"><td className="p-4 font-bold">React Dev Exam</td><td className="p-4">15 Questions</td><td className="p-4 text-center"><button className="text-blue-500 mx-2"><i className="bi bi-pencil"></i></button></td></tr>
              <tr className="border-b"><td className="p-4 font-bold">UI/UX Screening</td><td className="p-4">8 Questions</td><td className="p-4 text-center"><button className="text-blue-500 mx-2"><i className="bi bi-pencil"></i></button></td></tr>
            </tbody>
          </table>
...
const HRHiringProcess = () => {
  ...
  return (
    <div className="bg-white p-8 rounded-2xl shadow-lg overflow-x-auto">
      <h3 className="text-2xl font-bold text-space-blue mb-6 border-b pb-4">Hiring Process (Drag & Drop)</h3>
      <div className="flex gap-4 min-w-[800px]">
        {stages.map(stage => (
          <div key={stage} onDragOver={e => e.preventDefault()} onDrop={e => onDrop(e, stage)} className="flex-1 bg-gray-50 border border-gray-200 rounded-xl p-4 min-h-[400px]">
            <h4 className="font-bold text-space-blue text-center mb-4 border-b pb-2">{stage}</h4>
            <div className="space-y-3">
              {candidates.filter(c => c.stage === stage).map(c => (
                <div key={c.id} draggable onDragStart={e => onDragStart(e, c.id)} className="bg-white p-3 rounded shadow cursor-grab border border-gray-200 hover:border-mint-green hover:shadow-md transition-all">
                  <p className="font-bold text-sm text-space-blue">{c.name}</p>
                  <p className="text-xs text-gray-500 mb-2">{c.role}</p>
                  <button onClick={() => { setSelectedCand(c); setModalOpen(true); }} className="w-full bg-space-blue text-white text-xs py-1 rounded">Schedule / Alert</button>
                </div>
              ))}
            </div>
          </div>
...
const HRJobPosts = ({ setViewData }) => {
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
      <div className="bg-white p-8 rounded-2xl shadow-lg">
        <h3 className="text-2xl font-bold text-space-blue mb-6 border-b pb-4">Manage Job Posts</h3>
        <div className="bg-gray-50 p-6 rounded-xl border border-gray-200">
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div className="flex gap-4">
              <input type="number" placeholder="Min Salary" required className="w-1/2 border rounded-lg px-4 py-2" />
              <input type="number" placeholder="Max Salary" required className="w-1/2 border rounded-lg px-4 py-2" />
            </div>
      ...
      <div className="bg-white p-8 rounded-2xl shadow-lg flex flex-col">
        <h3 className="text-2xl font-bold text-space-blue mb-6 border-b pb-4">Your Posts</h3>
        <ul className="space-y-4 flex-grow">
          {posts.map(p => (
            <li key={p.id} className="border border-gray-200 rounded-xl p-4 flex justify-between items-center shadow-sm">
              <div>
                <p className="font-bold text-space-blue">{p.title}</p>
              ...
              <button onClick={() => setViewData({ type: 'hr-job-details', payload: p })} className="bg-space-blue text-white text-sm px-4 py-2 rounded-lg font-bold">View</button>
            </li>
          ))}
        </ul>
...
const HRJobDetailsPage = ({ job, goBack }) => (
  <div className="bg-white p-8 rounded-2xl shadow-lg">
    <button onClick={goBack} className="text-gray-500 hover:text-space-blue font-bold text-sm mb-6 flex items-center gap-2"><i className="bi bi-arrow-left"></i> Back to Posts</button>
    <h2 className="text-3xl font-bold text-space-blue">{job.title}</h2>
...
const AdminPortal = () => {
  ...
  return (
    <div className="flex h-screen bg-light-gray overflow-hidden">
      ...
      {/* Main Content Area */}
      <div className="flex-grow overflow-y-auto relative bg-gray-50 flex flex-col">
        {/* Topbar */}
        <div className="bg-white border-b px-8 py-4 flex justify-between items-center sticky top-0 z-30 shadow-sm">
          <h1 className="text-2xl font-bold text-space-blue capitalize">
            {viewData ? viewData.type.replace('-', ' ') : activeMenu.replace('-', ' ')}
          </h1>
          <div className="flex items-center gap-6">
            {/* Notification Bell */}
            <div className="relative">
              <button onClick={() => setShowNotifications(!showNotifications)} className="relative text-gray-500 hover:text-space-blue transition-colors">
              ...
              <AnimatePresence>
                {showNotifications && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-4 w-72 bg-white rounded-xl shadow-2xl border py-2 z-50">
        ...
        <div className="p-8">
          <AnimatePresence mode="wait">
            <motion.div key={viewData ? viewData.type : activeMenu} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
